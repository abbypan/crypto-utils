...
void main( int argc, char *argv[] )
{
    ...
    unsigned char *data;
    size_t data_len;
    read_file(argv[4], &data, &data_len);
    ...
    aes_gcm_encrypt(k, k_len, iv, iv_len, data, data_len, aad, &cipher_info, &cipher_info_len, &tag, tag_len);

    write_file(argv[5], cipher_info, cipher_info_len);

    write_file(argv[6], tag, tag_len);
